...
def load_jsonl(jsonl_path, filter_source=None):
    dataset = []
    with open(jsonl_path, "r", encoding="utf-8") as f:
        for line in f:
            item = json.loads(line)
            if filter_source is None or item.get("source") == filter_source:
                dataset.append(item)
    return dataset
...
def evaluate_dataset(args, eval_config):
    ...
    data_source = eval_config.get("source", None)
    ...
    dataset = load_jsonl(dataset_path, filter_source=data_source)
    random.seed(42)
...
def main():
    ...
    if args.batch_config is not None:
        eval_configs = load_config(args.batch_config)
    else:
